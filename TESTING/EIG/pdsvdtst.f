...
*
*     Disabling check for M and N in Test directory
...
*         If there is safe exit from API; pass this case
     ...
     $            DELTA, HETERO
            WRITE( NOUT, FMT = 9998) 'PDGESVD'
         END IF
         GO TO 120
      END IF
*
*       If M < 0 in SVD.dat file then PDGESVD API sets DINFO = -3
*       If N < 0 in SVD.dat file then PDGESVD API sets DINFO = -4
*
      IF ( DINFO.LT.0 ) THEN
         IF( IAM.EQ.0 ) 
