*
*
      SUBROUTINE PDSTEDCREQ( HETERO, NIN, MEM, MEMSIZE, NOUT, ISEED,
...
*  =======
*
*  PDSTEDCREQ performs one request from the input file 'STED.dat'
*  A request is the cross product of the specifications in the
*  input file.  PDSTEDCREQ prints one line per test.
*
*  Arguments
*  =========
*
*  NIN      (local input) INTEGER
*           The unit number for the input file 'STED.dat'
*
...
*     .. External Subroutines ..
     ...
     $                   DESCINIT, PDLASIZESQP, PDSTEDCINFO, PDSTEDCTST
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     ..
...
*
      CALL PDSTEDCINFO( INITCON, IAM, NIN, NOUT, MAXSETSIZE, NMATSIZES,
...
C    $                           IPOSTPAD + 1
                        ...
                        IF( LLWORK.LT.SIZETST ) THEN
                           NSKIPPED = NSKIPPED + 1
                        ELSE
                           CALL PDSTEDCTST( DESCA, UPLOS( UPLO ), N,
...
*                      complete at this stage
                           ...
                           ELSE IF( RES.EQ.3 ) THEN
                              NSKIPPED = NSKIPPED + 1
                              WRITE( NOUT, FMT = * )' PDSTEDCREQ failed'
