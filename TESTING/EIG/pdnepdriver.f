...
#ifndef DYNAMIC_WORK_MEM_ALLOC
      DOUBLE PRECISION   MEM( MEMSIZ ), CTIME( 1 ), WTIME( 1 )
#else
...
*                 If N < 0 in NEP.dat file then PDLAHQR API sets
*                 INFO = -5
                  IF (N.LT.0 .AND. INFO.EQ.-5) THEN
*                    If PDLAHQR is returning correct error
*                    code we need to pass this case
                     WRITE( NOUT, FMT = 9983 ) 'PDLAHQR'
...
*                          Re-enable EX_FLAG
     ...
     $                         INFO .EQ. -804 )) THEN
*                    When N < 0/Invalid, PDLAHQR INFO = -1
