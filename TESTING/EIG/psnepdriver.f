...
*                 If N < 0 in NEP.dat file then PSLAHQR API
*                 sets INFO = -5
                  IF ( N.LT.0 .AND. INFO.EQ.-5 )  THEN
*                    If PSLAHQR is returning correct error
*                    code we need to pass this case
                   WRITE( NOUT, FMT = 9983 ) 'PSLAHQR'
...
*                    Re-enable EX_FLAG
     ...
     $                        INFO .EQ. -804 )) THEN
*                    When N < 0/Invalid, PSLAHQR INFO = -1
