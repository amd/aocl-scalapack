...
*
                           CALL PDGEQRRV( M, N, MEM( IPA ), 1, 1,
     $                                DESCA, MEM( IPTAU ), MEM( IPW ) )
                           CALL PDLAFCHK( 'No', 'No', M, N, MEM( IPA ),
     $                              1, 1, DESCA, IASEED, ANORM, FRESID,
     $                              MEM( IPW ) )
                        ELSE IF( LSAMEN( 2, FACT, 'QL' ) ) THEN
*
*                          Compute residual = ||A-Q*L|| / (||A||*N*eps)
*
                           CALL PDGEQLRV( M, N, MEM( IPA ), 1, 1,
     $                                 DESCA,
     $                                 MEM( IPTAU ), MEM( IPW ) )
